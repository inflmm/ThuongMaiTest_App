...
function renderGallery(product) {
    ...
    masterContainer.innerHTML = masterFiles.map((fileName, index) => {
        const compactFile = fileName.replace('_master.jpg', '_master.jpg');
        const fullPath = joinUrl(API_BASE_URL, `${folderPath}/master/${compactFile}`);
...
function openOverlay(index) {
    ...
    if (overlayImageContainer) {
        overlayImageContainer.innerHTML = ''; // Clear ảnh cũ của sản phẩm trước đó (nếu có)
        
        currentProduct.images.forEach((image, i) => {
            const masterImgPath = joinUrl(API_BASE_URL, `${image}`);
            
            const imgEl = document.createElement('img');
            imgEl.src = masterImgPath;
            imgEl.className = 'overlay-master-item';
            imgEl.id = `overlay-img-${i}`;
            
            // Chỉ hiện ảnh được click mở ban đầu, các ảnh khác ẩn ngầm đi
            imgEl.style.display = (i === overlayIndex) ? 'block' : 'none';
            
            // GIỮ NGUYÊN HOẶC THÊM CLASS ZOOM CỦA BẠN VÀO ĐÂY
            // Ví dụ nếu bạn có logic zoom: imgEl.classList.add('zoomable');
            
            overlayImageContainer.appendChild(imgEl);
        });
    }
